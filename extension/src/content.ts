...
type StoredValues = {
	[key: string]: Partial<RecorderSettings> | undefined;
};
...
function readSettings(): Promise<RecorderSettings> {
	return new Promise((resolve) => {
		chrome.storage.local.get(SETTINGS_KEY, (values: StoredValues) => {
			resolve({
				...DEFAULT_SETTINGS,
				...(values[SETTINGS_KEY] ?? {}),
			});
...
function writeSettings(settings: RecorderSettings): Promise<void> {
	return new Promise((resolve) => {
...
class SelenaRecorderOverlay {
	...
	private container = createElement("section", "selena");
	private statusLabel = createElement("p", "selena__status", "Ready to record this page.");
	private timerLabel = createElement("div", "selena__timer", "00:00");
	private startButton = createElement("button", "selena__button selena__button--primary");
	private stopButton = createElement("button", "selena__button selena__button--secondary");
	private compactStopButton = createElement("button", "selena__button selena__button--primary selena__compact-stop");
	private downloadLink = createElement("a", "selena__download", "Download video");
	private closeButton = createElement("button", "selena__close", "×");
	private hint = createElement(
		"p",
		"selena__hint",
		"Pick the tab, window, or screen you want Selena to record. Audio support depends on what Chrome and the selected source allow.",
	);
	private tabAudioToggle = this.createToggle("Tab audio", "Include shared audio when Chrome provides it.");
	private microphoneToggle = this.createToggle("Microphone", "Mix your microphone into the recording.");
	private cameraToggle = this.createToggle("Camera", "Add a camera bubble to the final video.");
	private cameraShell = createElement("div", "selena__camera-shell");
	private cameraPreview = createElement("video", "selena__camera");
	...
	private createToggle(label: string, note: string): HTMLLabelElement {
		const wrapper = createElement("label", "selena__option");
		const copy = createElement("span", "selena__option-copy");
		copy.append(
			createElement("span", "selena__option-label", label),
			createElement("span", "selena__option-note", note),
		);
	...
	private createBrand(): HTMLDivElement {
		const brand = createElement("div", "selena__brand");
		brand.append(
			createElement("span", "selena__eyebrow", "HeySalad"),
			createElement("h2", "selena__title", "Selena"),
		);
		return brand;
	}

	private createIcon(kind: "record" | "stop"): HTMLSpanElement {
		return createElement("span", `selena__icon selena__icon--${kind}`) as HTMLSpanElement;
	}

	private getToggleInput(toggle: HTMLLabelElement): HTMLInputElement {
		return toggle.querySelector("input") as HTMLInputElement;
	}
	...
	private startDragging(event: PointerEvent): void {
		...
		if (target?.closest(".selena__close")) return;
	...
	private async getOptionalUserStream(settings: RecorderSettings): Promise<MediaStream | null> {
		if (!settings.includeMicrophone && !settings.includeCamera) return null;
	...
	private async buildComposedStream(settings: RecorderSettings): Promise<MediaStream> {
		...
		if (!context) throw new Error("Could not start Selena video compositor.");

		const drawFrame = () => {
			if (!context || !this.compositorCanvas || !this.displayVideo) return;
	...
	private async buildAudioTracks(settings: RecorderSettings): Promise<MediaStreamTrack[]> {
	...
	private startMediaRecorder(): void {
		if (!this.composedStream) throw new Error("No composed stream available.");
	...
	private attachEndHandlers(): void {
...
function getOverlay(): SelenaRecorderOverlay {
	const overlayWindow = window as Window & { selenaRecorderOverlay?: SelenaRecorderOverlay };
	if (!overlayWindow.selenaRecorderOverlay) overlayWindow.selenaRecorderOverlay = new SelenaRecorderOverlay();
	return overlayWindow.selenaRecorderOverlay;
}

chrome.runtime.onMessage.addListener((message) => {
	if (message?.type !== "selena:toggle-overlay") return;
